...
import { ArrowLeft, ArrowUpDown, ExternalLink, Star, GitFork } from "lucide-react";
import { useRunRepos } from "@/hooks/useSignalPipeline";
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              ...
              {netFilter !== "all" && ` in ${NET_LABELS[netFilter] || netFilter}`}
            </p>
          </div>
        </div>
              ...
              <TableBody>
                {filtered.map((repo) => (
                  <TableRow key={repo.id}>
                    <TableCell>
                      <a
                        href={repo.metadata?.html_url || `https://github.com/${repo.full_name}`}
                        target="_blank"
                    ...
                    <TableCell className="text-sm text-muted-foreground">
                      {repo.metadata?.pushed_at
                        ? new Date(repo.metadata.pushed_at).toLocaleDateString()
                        : "—"}
                    </TableCell>
                    <TableCell>
                      <div className="flex flex-wrap gap-1">
                        {((repo.metadata?.matched_nets as string[]) || []).map(
                          (n) => (
